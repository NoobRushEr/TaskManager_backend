...
public class TaskService : ITaskService
{
    ...
    public async Task<IEnumerable<TaskResponseDto>> GetAllTasksAsync()
    {
        ...
        return tasks.Select(task => MapToResponseDto(task!));
    }
    ...
    public async Task<TaskResponseDto?> UpdateTaskAsync(int id, UpdateTaskDto updateTaskDto, int userId, bool isAdmin)
    {
        ...
        if (existingTask.IsArchived) throw new InvalidOperationException("Archived tasks are read-only.");
    ...
    public async Task DeleteTaskAsync(int id, int userId, bool isAdmin)
    {
        var task = await _taskRepository.GetByIdAsync(id);
        if (task == null) return;
        if (task.IsArchived) throw new InvalidOperationException("Archived tasks are read-only.");
    ...
    {
        var tasks = await _taskRepository.GetMyTasksAsync(userId, includeDeleted, includeArchived);
        return tasks.Select(task => MapToResponseDto(task));
    }
    ...
    {
        await _taskRepository.ArchiveCompletedTasksAsync(cancellationToken);
    }
